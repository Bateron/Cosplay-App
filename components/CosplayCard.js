          ...
          <View style={[styles.progressOverlay, { backgroundColor: `rgba(0,0,0,0.4)` }]}>
            <View style={[styles.progressBar, { backgroundColor: theme.border }]}>
              <View
                style={[
                  styles.progressFill,
                  { backgroundColor: theme.success, width: `${progressPercentage}%` },
                ]}
              />
            </View>
      ...
      <View style={{ padding: spacing[4] }}>
        ...
        <View style={styles.metaGrid}>
          {cosplay.deadline && (
            <View style={styles.metaItem}>
              <Text style={{ fontSize: fontSize.xs, fontWeight: fontWeight.semibold, color: theme.textTertiary, textTransform: "uppercase", marginBottom: spacing[1] }}>
                Deadline
              </Text>
              <Text style={{ fontSize: fontSize.base, fontWeight: fontWeight.medium, color: theme.primary }}>
                {cosplay.deadline}
              </Text>
            </View>
          )}
          {cosplay.location && (
            <View style={styles.metaItem}>
              ...
              <Text style={{ fontSize: fontSize.base, fontWeight: fontWeight.medium, color: theme.text }} numberOfLines={1}>
                {cosplay.location}
              </Text>
            </View>
          )}
        ...
        >
          <View style={styles.costRow}>
            <View>
              <Text style={{ fontSize: fontSize.xs, fontWeight: fontWeight.semibold, color: theme.primaryDark, textTransform: "uppercase", marginBottom: spacing[1] }}>
                Total Budget
              </Text>
              <Text style={{ fontSize: fontSize.xl, fontWeight: fontWeight.bold, color: theme.primary }}>
                ₱{totalCost.toFixed(2)}
              </Text>
            </View>
            ...
            <View>
              ...
              <Text style={{ fontSize: fontSize.xl, fontWeight: fontWeight.bold, color: remainingCost > 0 ? theme.warning : theme.success }}>
        ...
        <View style={{ marginTop: spacing[4] }}>
          ...
          >
            ...
            <Text style={{ fontSize: fontSize.lg, color: theme.primary }}>
              {eventsOpen ? "▲" : "▼"}
            </Text>
          </TouchableOpacity>
          ...
          {eventsOpen && (
            ...
            >
              {eventsLoading ? (
                ...
                <View style={{ padding: spacing[4], alignItems: "center" }}>
                  <Text style={{ fontSize: fontSize.sm, color: theme.textSecondary, marginBottom: spacing[3] }}>
                    No events yet for this cosplay.
                  </Text>
                  <Button
                    title="+ Add Event"
                    variant="secondary"
                    size="sm"
                    onPress={() =>
                      navigation.navigate("Add Event", { cosplayId: cosplay.id, cosplayName: cosplay.characterName })
                    }
                  />
                </View>
              ) : (
                    ...
                    >
                      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start" }}>
                        <View style={{ flex: 1, marginRight: spacing[2] }}>
                          <Text style={{ fontSize: fontSize.base, fontWeight: fontWeight.bold, color: theme.text, marginBottom: spacing[1] }}>
                            {event.name}
                          </Text>
                          {event.date ? (
                            <Text style={{ fontSize: fontSize.sm, color: theme.primary, marginBottom: spacing[1] }}>
                              📆 {event.date}
                            </Text>
                          ) : null}
                          {event.location ? (
                            <Text style={{ fontSize: fontSize.sm, color: theme.textSecondary, marginBottom: spacing[1] }}>
                              📍 {event.location}
                            </Text>
                          ) : null}
                          {event.description ? (
                            <Text style={{ fontSize: fontSize.xs, color: theme.textTertiary }} numberOfLines={2}>
                              {event.description}
                            </Text>
                          ) : null}
                        </View>
...
const styles = StyleSheet.create({
  ...
  progressOverlay: { position: "absolute", bottom: 0, left: 0, right: 0, paddingVertical: 12, paddingHorizontal: 16 },
  ...
  metaGrid: { flexDirection: "row", gap: 16, marginBottom: 8 },
  metaItem: { flex: 1 },
